...
represented on the modal ring. We define mode coverage by assigning each sample
...
normalized mode entropy, and minimum mass per mode; KSD and mean log
...
it does not match the adaptive variant on the main multimodal metrics. SGLD and
SGHMC remain trapped in a single mode, with final coverage fixed at $0.125$ and
zero mode entropy. The minimum mass statistic is zero for all methods at this
budget, so in this redesigned benchmark coverage and entropy are the
informative multimodal diagnostics while KSD remains a secondary stability
measure.
