...
with checkpointing every 20 steps across 5 seeds. We define mode coverage by
assigning each sample to its nearest mixture center and counting a mode as
covered once it carries at least 5\% of the empirical mass. The primary metrics
are mode coverage, normalized mode entropy, and minimum mass per mode; KSD and
mean log probability are retained as secondary diagnostics, with ESS reported as
a supplemental mixing proxy.
...
\begin{table}[t]
  ...
  \begin{tabular}{lrrr}
    ...
    \midrule
    Adapt-MR-SVGD & 0.400 & 0.461 & 0.317 \\
    SVGD          & 0.325 & 0.375 & 6153.2 \\
    Strang-SVGD   & 0.250 & 0.311 & $1.17\times 10^{8}$ \\
    MR-SVGD       & 0.250 & 0.294 & 77.9 \\
...
Under this harder off-mode initialization, adaptive MR-SVGD gives the strongest
overall multimodal exploration at the fixed budget, attaining the highest mean
final mode coverage and mode entropy across seeds; \cref{tab:mix8_summary}
provides the corresponding mean-over-seeds summary, while
\cref{fig:mix8_summary} carries the cross-seed variability. Vanilla SVGD
occasionally reaches comparable coverage on individual seeds, but does so with
extremely poor KSD and mean log probability, indicating unstable mass
allocation rather than reliable exploration. Fixed MR-SVGD improves over the
chain baselines but does not match the adaptive variant. SGLD and SGHMC remain
trapped in a single mode, with final coverage fixed at $0.125$ and zero mode
entropy. The minimum mass statistic is zero for all methods at this budget, so
in this redesigned benchmark coverage and entropy are the informative
multimodal diagnostics while KSD remains a secondary stability measure.
...
benchmark supportive rather than decisive: multirate structure improves
