...
# Fast dynamics: Gradient of the log target distribution
def fast_dynamics(y, t):
    """Fast dynamics: Gradient of the log target distribution."""
    return grad_log_p(y)

# Slow dynamics: Random noise
def random_noise_step(current_position, step_size):
    """Slow component: Random noise."""
    return current_position + step_size * np.random.randn(*current_position.shape)

# MIS-integrated MALA step with swapped dynamics
def mala_step_mis_swapped(current_position, step_size, dt_fast=0.01, dt_slow=0.1):
    # Fast dynamics: Integrate gradient over finer time steps
    t_fast = np.arange(0, dt_slow, dt_fast)
    gradient_trajectory = odeint(fast_dynamics, current_position, t_fast)
    gradient_update = gradient_trajectory[-1]  # Final value after integration

    # Slow dynamics: Random noise
    noise_update = random_noise_step(gradient_update, step_size)
    
    # Metropolis-Hastings acceptance step
    current_log_prob = np.log(target_distribution(current_position) + 1e-9)
    proposal_log_prob = np.log(target_distribution(noise_update) + 1e-9)
    
    # Transition probabilities
    proposal_to_current = -np.sum((current_position - gradient_update)**2) / (2 * step_size**2)
    current_to_proposal = -np.sum((noise_update - gradient_update)**2) / (2 * step_size**2)
...
def plot_particles(samples, target_distribution, iteration):
    ...
    plt.title(f'MALA Samples with MIS (Swapped Dynamics) at Iteration {iteration}')
    plt.xlim(-5, 5)
    plt.ylim(-5, 5)
    plt.xlabel('x1')
    plt.ylabel('x2')
...
# Perform MALA sampling with MIS and swapped dynamics
num_iterations = 1000
step_size = 0.5
...
for i in range(num_iterations):
    current_position = mala_step_mis_swapped(current_position, step_size, dt_fast, dt_slow)
