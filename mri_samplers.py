...
class TargetDistribution:
    def __init__(self, target, device):
        ...
        self.distributions = {
            ...
            "checkerboard": (
                self.checkerboard_distribution,
                self.checkerboard_grad_log_p,
            ),
            "v^": (
                self.curved_gaussian_mixture,
                self.curved_gaussian_mixture_grad_log_p,
            ),
        }
    ...
    def curved_gaussian_mixture(
        ...
        gauss1 = torch.exp(
            -0.5
            * (((x1 - mu1[0]) / sigma1) ** 2 + ((x2_curved - mu1[1]) / sigma1) ** 2)
        ) / (2 * torch.pi * sigma1**2)
        gauss2 = torch.exp(
            -0.5
            * (((x1 - mu2[0]) / sigma2) ** 2 + ((x2_curved - mu2[1]) / sigma2) ** 2)
        ) / (2 * torch.pi * sigma2**2)

        # Weighted sum of components
        return weight1 * gauss1 + weight2 * gauss2

    def curved_gaussian_mixture_grad_log_p(
        self,
        x,
        mu1=torch.tensor([-2.0, -2.0]),
        mu2=torch.tensor([2.0, 2.0]),
    ...
    def spirals_grad_log_p(self, x):
        ...
        r = torch.sqrt(x[:, 0] ** 2 + x[:, 1] ** 2)
        grad_r = (
            (r - (theta / (2 * torch.pi) * 4)) * x / r.unsqueeze(1)
        )  # Decreased radius
        grad_theta = (
            (r - (theta / (2 * torch.pi) * 4))
    ...
    def gaussian_distribution(self, x):
        ...
        mean2, cov2 = torch.tensor([-2.0, -2.0]), torch.tensor(
            [[0.5, -0.1], [-0.1, 0.5]]
        )
        pdf1 = torch.exp(
            -0.5 * torch.sum((x - mean1) @ torch.linalg.inv(cov1) * (x - mean1), dim=-1)
        ) / torch.sqrt(torch.det(cov1) * (2 * torch.pi) ** 2)
        pdf2 = torch.exp(
            -0.5 * torch.sum((x - mean2) @ torch.linalg.inv(cov2) * (x - mean2), dim=-1)
        ) / torch.sqrt(torch.det(cov2) * (2 * torch.pi) ** 2)
    ...
    def gaussian_grad_log_p(self, x):
        ...
        d_log_p = (
            0.5
            * (grad1 * pdf1[:, None] + grad2 * pdf2[:, None])
            / (pdf1 + pdf2).unsqueeze(-1)
        )
    ...
    def checkerboard_distribution(self, x):
        ...
        checkered = (
            (torch.floor(x1 / scale) + torch.floor(x2 / scale)) % 2 == 0
        ).float()
        return checkered
...
class SVGDSampler(SamplingMethod):
    def run(self, metrics_tracker, step_size=0.05):
        ...
        while t < self.t_final:
            ...
            kernel_grad = (
                -(self.particles.unsqueeze(1) - self.particles.unsqueeze(0))
                * kernel_matrix.unsqueeze(-1)
                / (self.bandwidth**2)
            )
            ...
            phi = (kernel_matrix @ grad_log_p_values) / num_particles + kernel_grad.sum(
                dim=0
            ) / num_particles
            ...
            metrics_tracker.track(
                ...
                self.counter.get_flops(),
                t.item(),
            )
            all_particles.append(self.particles.clone())

            self.counter.count_addition(
                self.particles.unsqueeze(1), self.particles.unsqueeze(0)
            )
    ...
    def run(self, metrics_tracker, step_size=0.01):
        ...
        while t < self.t_final:
            ...
            def f_fast(t, particles):
                kernel_grad = (
                    -(particles.unsqueeze(1) - particles.unsqueeze(0))
                    * kernel_matrix.unsqueeze(-1)
                    / (self.bandwidth**2)
                )
            ...
            relative_error = torch.norm(ynext - yhat) / torch.clamp(
                torch.norm(ynext), min=1e-6
            )
...
class MRSampler(SamplingMethod):
    def run(self, metrics_tracker, step_size=0.05):
        ...
        while t < self.t_final:
            ...
            ratio = repulsion.abs() / torch.min(
                drift.abs(), 1e-9 * torch.ones_like(drift)
            )
            # mask  = ratio > 10
            # print('mask', mask)
...
def mriGARKstep(particles, f_slow, f_fast, step_size):
    ...
    v2 = torchdiffeq.odeint(
        stage_ode,
        v1,
        torch.tensor([0, step_size], device=particles.device),
        rtol=1e-3,
        atol=1e-3,
        options=ode_options,
    )[-1, :, :]
...
class SamplingExperiment:
    def __init__(
        self, target_distribution, grad_log_p, particles, t_final, bandwidth=0.5
    ):
        self.target_distribution = target_distribution
        self.grad_log_p = grad_log_p
        self.particles = particles
...
class MetricsTracker:
    ...
    def kl_divergence(self, particles, target_distribution, bandwidth=0.5):
        ...
        particle_density = kernel_matrix.sum(dim=1) / (
            N * (2 * torch.pi * bandwidth**2) ** (D / 2)
        )
        target_density = (
            target_distribution(particles.unsqueeze(0)).squeeze().clamp(min=1e-12)
        )
