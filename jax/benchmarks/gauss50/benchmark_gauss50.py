...
SEEDS = range(5)                # Seed ablation (train/test split analog)
...
with open(out_csv, "w", newline="") as f:
    ...
    writer.writerow([
        "method", "seed", "iter", "grad_evals", "kernel_evals", "wall_s",
        "mu_err", "cov_err", "ess", "ksd", "mean_logp",
        "nonfinite_frac", "stiff_ratio", "m_used", "is_best",
    ])
    ...
    for seed in SEEDS:
        ...
        x0_chain = jax.random.uniform(chain_key, (50,), minval=-4.0, maxval=4.0)
        # Starting particles for ensemble methods (SVGD variants)
        init_particles = jax.random.uniform(init_key, (N_particles, 50), minval=-4.0, maxval=4.0)
        ...
        for name, (state, step_fn) in samplers.items():
            ...
            for it in trange(1, n_iter + 1):
                ...
                if it % save_every == 0:
                    ...
                    row = [
                        name, seed, it, grad_evals, kernel_evals, elapsed,
                        mu_err_val, cov_err_val, ess_val, ksd_val, mlp_val,
                        nonfinite_frac, stiff_ratio, m_used, 0,
                    ]
                    writer.writerow(row)
                    ...
                    if EARLY_STOP:
                        ...
                        if check_count >= EARLY_STOP_MIN_CHECKS and bad_checks >= EARLY_STOP_PATIENCE:
                            print(
                                f"early stop: seed {seed} | {name} at iter {it} "
